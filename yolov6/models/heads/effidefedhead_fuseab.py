...
class Detect(nn.Module):
    ...
    '''
    def __init__(self, num_classes=80, anchors=None, num_layers=3, inplace=True, head_layers=None, use_dfl=True, fed_training = False, reg_max=16, num_emotions = 7):  # detection layer
        super().__init__()
        assert head_layers is not None
        self.nc = num_classes  # number of classes
        self.no = num_classes + 5  # number of outputs per anchor
        self.nl = num_layers  # number of detection layers
        self.ne = num_emotions
        ...
        self.cls_ped_convs = nn.ModuleList()
        self.reg_convs = nn.ModuleList()
        self.cls_preds = nn.ModuleList()
        self.cls_ped_preds = nn.ModuleList()
        ...
        for i in range(num_layers):
            ...
            self.cls_ped_convs.append(head_layers[idx+2])
            self.reg_convs.append(head_layers[idx+3])
            self.cls_preds.append(head_layers[idx+4])
            self.cls_ped_preds.append(head_layers[idx+5])
    ...
    def initialize_biases(self):
        ...
        for conv in self.cls_ped_preds:
            b = conv.bias.view(-1, )
            b.data.fill_(-math.log((1 - self.prior_prob) / self.prior_prob))
            conv.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
            w = conv.weight
        ...
        for conv in self.cls_ped_preds_ab:
            b = conv.bias.view(-1, )
            b.data.fill_(-math.log((1 - self.prior_prob) / self.prior_prob))
            conv.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
            w = conv.weight
    ...
    def forward(self, x):
        if self.training:
            device = x[0].device
            cls_score_list_af = []
            cls_ped_score_list_af = []
            reg_dist_list_af = []
            cls_score_list_ab = []
            cls_ped_score_list_ab = []
            reg_dist_list_ab = []
            ...
            for i in range(self.nl):
                ...
                cls_ped_feat = self.cls_ped_convs[i](cls_ped_x)
                reg_feat = self.reg_convs[i](reg_x)

                #anchor_base
                cls_output_ab = self.cls_preds_ab[i](cls_feat)
                cls_ped_output_ab = self.cls_ped_preds_ab[i](cls_ped_feat)
            ...
            for i in range(self.nl):
                ...
                cls_feat = self.cls_convs[i](cls_x)
                cls_ped_feat = self.cls_ped_convs[i](cls_x)
                reg_feat = self.reg_convs[i](reg_x)

                #anchor_free
                cls_output_af = self.cls_preds[i](cls_feat)
                cls_ped_output_af = self.cls_ped_preds[i](cls_feat)
                reg_output_af = self.reg_preds[i](reg_feat)
                ...
                if self.export:
                    ...
                    cls_ped_score_list_af.append(cls_ped_output_af)
                    reg_dist_list_af.append(reg_output_af)
                else:
                    cls_score_list_af.append(cls_output_af.reshape([b, self.nc, l]))
                    cls_ped_score_list_af.append(cls_ped_output_af.reshape([b, self.ne, l]))
            ...
            cls_ped_score_list_af = torch.cat(cls_ped_score_list_af, axis=-1).permute(0, 2, 1)
            ...
            cls_ped_score_list = cls_ped_score_list_af
            return torch.cat(
                [
                    pred_bboxes,
...
def build_effidehead_layer(channels_list, num_anchors, num_classes, num_emotions, reg_max=16, num_layers=3):
    ...
    head_layers = nn.Sequential(
        ...
        nn.Conv2d(
            in_channels=channels_list[chx[0]],
            out_channels=num_emotions * num_anchors,
            kernel_size=1
        ),
        # reg_pred0_3ab
        nn.Conv2d(
