...
class ComputeLoss:
    ...
    def __init__(self,
        ...
        self.varifocal_loss = VarifocalLoss(class_weights).cuda()
...
class VarifocalLoss(nn.Module):
    ...
    def forward(self, pred_score,gt_score, label, alpha=0.75, gamma=2.0):
        ...
        with torch.amp.autocast('cuda', enabled=False):
            loss = (F.binary_cross_entropy(pred_score.float(), gt_score.float(), reduction='none', weight=self.class_weights) * weight).sum()
